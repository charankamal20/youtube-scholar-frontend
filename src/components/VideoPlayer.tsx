...
const YouTubePlayer = ({
  ...
  function PushToArray(data: z.infer<typeof FormSchema>) {
    const newNote: Note = {
      id: Math.random().toString(),
      title: data.title,
      body: data.note,
      timestamp: currentTime?.seconds || 0,
      timestampDisplay: currentTime?.displayTime || "",
    };
    ...
    <div className="w-full h-full object-cover ">
      ...
      <div className="flex px-1 items-center justify-between py-2">
        <div className="relative">
          <div className={`h-full w-full absolute bg-background duration-500 ${zenMode ? "opacity-100 z-10":"opacity-0 -z-10"}`}></div>
          <Link
            target="_blank"
